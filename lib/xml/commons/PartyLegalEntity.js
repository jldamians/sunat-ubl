...
class PartyLegalEntity extends Element {
  ...
  _registrationAddress() {
    ...
    if (this._urbanizacion !== null && utils.isUndefined(this._urbanizacion) === false) {
      RegistrationAddress
        .ele(`${super.prefix.cbc}:CitySubdivisionName`)
          .dat(this._urbanizacion);
    }

    if (this._provincia !== null && utils.isUndefined(this._provincia) === false) {
      RegistrationAddress
        .ele(`${super.prefix.cbc}:CityName`)
          .dat(this._provincia);
    }

    if (this._departamento !== null && utils.isUndefined(this._departamento) === false) {
      RegistrationAddress
        .ele(`${super.prefix.cbc}:CountrySubentity`)
          .dat(this._departamento);
    }

    if (this._distrito !== null && utils.isUndefined(this._distrito) === false) {
      RegistrationAddress
    ...
    if (this._codPais !== null && utils.isUndefined(this._codPais) === false) {
      RegistrationAddress
  ...
  toXMLElement() {
    ...
    const hasOriginAddress = (
      ...
      (this._codPais !== null && utils.isUndefined(this._codPais) === false) ||
      (this._urbanizacion !== null && utils.isUndefined(this._urbanizacion) === false) ||
      (this._provincia !== null && utils.isUndefined(this._provincia) === false) ||
      (this._departamento !== null && utils.isUndefined(this._departamento) === false) ||
      (this._distrito !== null && utils.isUndefined(this._distrito) === false)
    );

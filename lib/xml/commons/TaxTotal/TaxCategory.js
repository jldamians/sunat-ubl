...
class TaxCategory extends Element {
  ...
  toXMLElement() {
    ...
    if (this.payload.tasaImpuesto !== null && utils.isUndefined(this.payload.tasaImpuesto) === false) {
      if (this.payload.tasaImpuesto >= 0) {
        TaxCategory.importDocument(this._percent());
      }
    }

    if (this.payload.codigoAfectacionIgv !== null && utils.isUndefined(this.payload.codigoAfectacionIgv) === false) {
      TaxCategory.importDocument(this._taxExemptionReasonCode());
    }

    if (this.payload.codigoSistemaCalculoIsc !== null && utils.isUndefined(this.payload.codigoSistemaCalculoIsc) === false) {

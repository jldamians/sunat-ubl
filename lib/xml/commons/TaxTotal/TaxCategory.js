...
class TaxCategory extends Element {
  ...
  toXMLElement() {
    ...
    if (this.payload.tasaImpuesto !== null && this.payload.tasaImpuesto !== undefined && this.payload.tasaImpuesto >= 0) {
      TaxCategory.importDocument(this._percent());
    }

    if (this.payload.codigoAfectacionIgv) {
      TaxCategory.importDocument(this._taxExemptionReasonCode());
    }

    if (this.payload.codigoSistemaCalculoIsc) {

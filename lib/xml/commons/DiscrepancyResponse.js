...
class DiscrepancyResponse extends Element {
    ...
    if (this._tipoComprobante === DOCUMENT_TYPE_CODES.ND) {
      if (this._debitNoteTypeAllowed(this._tipoNota) === true) {
        xml
          ...
          .att('listName', CATALOGS.getName(CATALOGS.CATALOG_10))
      ...
      if (this._creditNoteTypeAllowed(this._tipoNota) === true) {
        xml
          .att('listAgencyName', 'PE:SUNAT')
          .att('listName', CATALOGS.getName(CATALOGS.CATALOG_09))
   ...
   */
  _responseCode() {
    const responseCode = super.create(`${XMLNS.getPrefix(super.prefix.cbc)}ResponseCode`);
   ...
   */
  _description() {
    const description = super.create(`${XMLNS.getPrefix(super.prefix.cbc)}Description`);
  ...
  toXMLElement() {
    const discrepancyResponse = super.create(`${XMLNS.getPrefix(super.prefix.cac)}DiscrepancyResponse`);

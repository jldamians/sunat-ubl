...
class PaymentGenericTag extends Element {
  /**
   * @constructor
   * @param {ComprobanteAnticipo} cpe, información del comprobante
   * @param {Object} prefix, prefijos de los namespace
   */
  constructor(cpe, prefix, xmlName = null, ublVersion = null) {
    let newXmlName;

    if (xmlName === null) {
      newXmlName = `${XMLNS.getPrefix(prefix.cac)}PrepaidPayment`;
    } else {
  ...
  _setIDTag() {
    ...
    if (super.ublVersion === VERSIONS.UBL_2_0) {
      tag.xml.txt(this._cpe.identificador);
    } else {
      tag.xml
  ...
  _setPaidAmountTag() {
    ...
    tag.xml
      .att('currencyID', this._cpe.codMoneda)
      .txt(this._cpe.monto);
  ...
  _setPaidDateTag() {
    ...
    super.xml.importDocument(this._cpe.fechaPago);
  }

...
class LineGenericTag extends Element {
  ...
  _getTaxes() {
    ...
    const base = this._baseAfectacionIgv;
    const monto = this._mntAfectacionIgv;
    const tasa = this._tasaAfectacionIgv;
    const tipoAfectacionIgv = this._tipoAfectacionIgv;
  ...
  _pricingReferenceTag() {
    ...
    if (this._isCostless() === false ) {
      ...
      if (this._tipoPrecioUnitario !== SALE_PRICE_TYPE_CODES.VALOR) {
        throw new Error(
          `En operaciones "no onerosas" debe utilizar: ${SALE_PRICE_TYPE_CODES.PRECIO}(${SALE_PRICE_TYPE_CODES.getText(SALE_PRICE_TYPE_CODES.PRECIO)})`
        );
      }
    }

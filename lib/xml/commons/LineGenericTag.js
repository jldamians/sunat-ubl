...
class LineGenericTag extends Element {
  ...
  constructor(
    ...
    const isAffectationTypeAllowed = AFFECTATION_TYPES_CODES.checkCode(tipoAfectacion);
    ...
    this._mntValorVenta = mntValorVenta;
    ...
    this._tipoAfectacion = tipoAfectacion;
  ...
  /**
   * Determina si la operación es ONEROSA o GRATUITA
   * @return {Boolean}
   */
  _isCostless() {
    ...
    const costless = costlessAffectationTypeCodes.includes(this._tipoAfectacion);
  ...
  _getTributeTypeCode() {
    ...
    // Si el código no existe se lanzará una excepción
    AFFECTATION_TYPES_CODES.getText(this._tipoAfectacion);

    switch (this._tipoAfectacion) {
   ...
   */
  _getTaxes() {
    const codigo = this._getTributeTypeCode();

    const base = this._mntValorVenta;

    const tipoAfectacion = this._tipoAfectacion;
  ...
  _pricingReferenceTag() {
    ...
    // Si la operación es "onerosa", se deberá consignar el código "01".
    // Si la operación es "no onerosa", se deberá consignar el código "02".
    if (this._isCostless() === false ) {
      if (this._tipoPrecioUnitario !== SALE_PRICE_TYPE_CODES.PRECIO) {
        throw new Error(`Cuando la operación sea "onerosa", debe consignar el código "01"`);
      }
    } else {
      if (this._tipoPrecioUnitario !== SALE_PRICE_TYPE_CODES.VALOR) {
        throw new Error(`Cuando la operación sea "no onerosa", debe consignar el código "02"`);
      }
    }
   ...
   */
  _taxTotalTag() {
    const taxTotalTag = new TaxTotal();
  ...
  toXMLElement() {
    ...
    lineTag.importDocument(this._IDTag());

    lineTag.importDocument(this._quantityGenericTag());

    lineTag.importDocument(this._lineExtensionAmountTag());

    lineTag.importDocument(this._pricingReferenceTag());

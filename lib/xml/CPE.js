...
class Invoice extends Element {
  ...
  _setNoteTag() {
    ...
    this._payload.notas.forEach((nota) => {
      super.xml
        .ele(tagName)
          .att('languageLocaleID', nota.codNota)
  ...
  _setSupplierPartyTag() {
    ...
    const tag = new commons.PartyGenericTag(
      this._payload.emisor, prefix, 'AccountingSupplierParty', super.ublVersion
    );
  ...
  _setCustomerPartyTag() {
    ...
    const tag = new commons.PartyGenericTag(
      this._payload.receptor, prefix, 'AccountingCustomerParty', super.ublVersion
    );

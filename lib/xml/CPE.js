...
class Invoice extends Element {
  ...
  _setSignatureTag() {
    ...
    const signUri = `${this._payload.firmante.tipoDocumento}-${this._payload.firmante.numeroDocumento}`;

    const signId = (
      `${this._payload.emisor.tipoDocumento}-${this._payload.emisor.numeroDocumento}-${this._payload.tipoCpe}-${this._payload.serie}-${this._payload.numero}`
    );

    const signatureTag = new commons.Signature(
      signUri, signId, this._payload.firmante, prefix, super.ublVersion
    );

    super.xml.importDocument(signatureTag.toXMLElement());
  }
